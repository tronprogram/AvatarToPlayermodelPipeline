...
Public methods return named dataclasses, not dicts. Construct the service, call one method, read the fields on the result.
...
| `source_material_name` / `allocate_source_name` | `app.core.source_names` — Blender imports this (stdlib only) |
...
| `hlmv_gameinfo.txt` / `crowbar_settings.xml` | Crowbar / HLMV helpers |
